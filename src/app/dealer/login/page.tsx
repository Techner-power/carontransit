...
import { dealerSignIn } from "@/lib/dealerActions";
...
export default function DealerLoginPage() {
  ...
  const handleSubmit = (formData: FormData) => {
    ...
    startTransition(async () => {
      const result = await dealerSignIn(formData);
      if (!result.success) {
  ...
  return (
    <div className="max-w-[420px] mx-auto px-6 py-16">
      <h1 className="text-2xl font-extrabold mb-1">Dealer Login</h1>
      ...
      <p className="text-sm text-port-steel mt-6">
        ...
        <Link href="/dealer/signup" className="font-semibold text-ink-navy underline">
          Sign up
        </Link>
      </p>
    </div>
